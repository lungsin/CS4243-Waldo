...
def partial_matching_waldo_pyramid(image):
    template = cv2.imread(f"datasets/Features/partial_waldo.jpg")
    ...
    for i in pyramid(image, downscale=1.2, min_size=template.shape): 
        boxes = template_matching(i, template)
        for box in boxes:
            for idx in range(len(box)):
                box[idx] *= image.shape[0] / i.shape[0] 
        total.extend(boxes)
...
if __name__ == "__main__":
    ...
    total = partial_matching_waldo_pyramid(img)
    for j in total:
